...
def run_simulation(num_colonies, simulation_time, k1, k2, k3, kx, f_initial):
    """
    Run multiple colonies and calculate gene expressions for the case with irreversible active
    """
    ...
    for nc in range(num_colonies):
        ...
        times, _, state2_counts, state3_counts = simulation.simulate(simulation_time)
        ...
        for i, time in enumerate(times):
            if timer > 9:
                break
            if time >= timer:
                total_active_at_time = state2_counts[i] + state3_counts[i]
                totals[timer][nc] = total_active_at_time
                timer += 1
        
        # Calculate final gene expression of active cells
        total_active = state2_counts[-1] + state3_counts[-1]
